...
export const InputField = memo(function InputField({ onSendMessage, onKeyDown, disabled, isStreaming, onStop }: InputFieldProps) {
    ...
    }, [message, onSendMessage, disabled, isStreaming]);
    ...
    return (
        <div className="relative group/input">
            <div className={`relative flex items-center gap-2 p-2 bg-card/60 backdrop-blur-xl border border-border/80 rounded-[24px] shadow-lg ring-1 ring-border/20 focus-within:ring-primary/20 focus-within:border-primary/40 transition-all duration-500 hover:border-border ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <Button
                    size="icon"
                    variant="ghost"
                ...
                {isStreaming ? (
                    ...
                    <Button
                        size="icon"
                        className={`h-8 w-8 shrink-0 rounded-full transition-all duration-500 ${message.trim() && !disabled
